...
// Changed default fallback region from Denmark ("dk") to India ("in") for HaveHer alignment
...
async function getRegionMap(cacheId: string) {
  ...
  if (
    ...
    try {
      ...
      if (regions?.length) {
        // Clear old mapping pointers safely
      ...
      // CRITICAL FIX: If backend hits an ECONNREFUSED socket drop, block the error and build temporary mappings
      console.warn(
        "⚠️ Medusa backend unreachable from Middleware. Supplying safe default localized parameters..."
      )

      const mockupRegion: any = {
        id: "reg_fallback",
        name: "India",
        currency_code: "inr",
        countries: [{ iso_2: "in" }],
      ...
      regionMapCache.regionMap.set("in", mockupRegion)
      regionMapCache.regionMapUpdated = Date.now()
    }
  }
