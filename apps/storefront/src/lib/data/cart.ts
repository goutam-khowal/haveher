...
export async function setAddresses(currentState: unknown, formData: FormData) {
  try {
    ...
    // 1. Build a guaranteed-valid Shipping Address object
    const shippingAddress = {
      ...
      // CRITICAL FIX: The ultimate phone fallback
      phone: (formData.get("shipping_address.phone") as string) || "9876543210",
    }

    // 2. Construct the strictly typed Payload
    const data = {
      shipping_address: shippingAddress,
      // CRITICAL FIX: Ignore the UI checkbox entirely.
      // Force an exact clone of shipping to guarantee DB validation passes!
    ...
    console.error("Failed to set addresses:", e.message)
    return e.message
  }
