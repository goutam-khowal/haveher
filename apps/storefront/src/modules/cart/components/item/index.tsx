...
const Item = ({ item, type = "full", currencyCode }: ItemProps) => {
  ...
  return (
    <div
      className={clx(
        "w-full py-4 sm:py-6 font-sans border-b border-pink-100/50 last:border-none transition-opacity duration-200",
        { "opacity-60 pointer-events-none": updating }
      )}
      data-testid="product-row"
    >
      <div
        className={clx("w-full grid items-center", {
          "grid-cols-1 gap-y-4 sm:grid-cols-[1fr_auto_100px_100px] sm:gap-x-6":
            type === "full",
          "grid-cols-[1fr_auto] gap-x-6": type === "preview",
        })}
      >
        {/* AREA 1: Image & Meta Layout Details Container */}
        <div className="flex items-center gap-x-4 min-w-0">
          <div
            className={clx(
              "bg-gray-50 rounded-xl overflow-hidden shrink-0 border border-pink-50 shadow-3xs",
              {
                // 👑 BIGGER PRODUCT THUMBNAILS AT CHECKOUT: Upscaled to w-24 h-32 for ultimate portrait dress scaling
                "w-24 h-32": type === "preview",
                "w-16 h-20 sm:w-20 sm:h-24": type === "full",
              }
            )}
          >
            ...
            >
              <Thumbnail
                thumbnail={item.thumbnail}
                images={item.variant?.product?.images}
                size="full" // Set to full to avoid square bounding crop restrictions
              />
            </LocalizedClientLink>
          </div>
          ...
          <div className="flex flex-col min-w-0 text-left justify-center">
            <h3
              className={clx(
                "font-serif italic text-[#3A1A2A] font-medium tracking-wide truncate pr-2 hover:text-[#D45C88] transition-colors leading-snug",
                {
                  "text-base": type === "preview",
                  "text-base md:text-lg": type === "full",
                }
              )}
            >
              <LocalizedClientLink
        ...
        {/* PREVIEW LAYOUT COMPACT PRICE & QTY INFO SPLIT */}
        {type === "preview" && (
          <div className="flex flex-col items-end gap-y-1 text-xs font-sans min-w-[80px]">
            <span className="text-gray-400 uppercase tracking-widest text-[10px] font-bold">
              Qty: {item.quantity}
            </span>
            <span className="font-bold text-sm text-[#3A1A2A]">
              <LineItemPrice
                item={item}
                style="tight"
                currencyCode={currencyCode}
