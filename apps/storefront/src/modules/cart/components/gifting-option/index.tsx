...
const GiftingOption = ({ cart }: GiftingOptionProps) => {
  // Database state check parsing fallback
  const initialIsGift = !!cart?.metadata?.is_gift
  const initialMessage = (cart?.metadata?.gift_message as string) || ""

  const [isGift, setIsGift] = useState(initialIsGift)
  const [giftMessage, setGiftMessage] = useState(initialMessage)
  const [isPending, startTransition] = useTransition()

  const handleCheckboxChange = (checked: boolean) => {
    setIsGift(checked)
    startTransition(async () => {
      await toggleCartGifting(checked, giftMessage)
    })
  }
  ...
  const handleMessageBlur = () => {
    if (isGift) {
      startTransition(async () => {
        await toggleCartGifting(true, giftMessage)
      })
    }
  }
  ...
  return (
    <div
      className={`w-full bg-[#FDF1F6]/50 border border-[#F0C4D8]/30 rounded-2xl p-5 shadow-3xs mt-6 text-left ${
    ...
    >
      <label className="flex items-start gap-x-3.5 cursor-pointer select-none">
        ...
        <div className="flex flex-col min-w-0 cursor-pointer">
          <span className="font-serif italic text-base font-medium text-[#3A1A2A] tracking-wide flex items-center gap-x-2">
            Sending love to someone special?{" "}
            <span className="text-xs font-sans not-italic font-bold text-[#D45C88] bg-white border border-[#F0C4D8]/40 px-2 py-1.5 rounded-full ">
              ₹20
            </span>
          </span>
          <p className="text-xs text-gray-400 mt-0.5">
      ...
      {isGift && (
        <div className="mt-4 pt-4 border-t border-dashed border-[#F0C4D8]/40 flex flex-col gap-y-2">
          <label className="text-[10px] font-bold uppercase tracking-wider text-gray-400">
            Personalized Note Message *
          </label>
          <textarea
            value={giftMessage}
            disabled={isPending}
            onBlur={handleMessageBlur} // User jab text likhkar bahar tap karega, server save automatic trigger ho jayega
            onChange={(e) => setGiftMessage(e.target.value)}
            placeholder="Type your warm message here..."
            rows={3}
            maxLength={250}
