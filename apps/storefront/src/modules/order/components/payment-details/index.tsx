...
const PaymentDetails = ({ order }: PaymentDetailsProps) => {
  ...
  // Safe fallback to prevent crashes if the provider isn't in the default Medusa map
  const getPaymentInfo = (providerId: string) => {
    ...
    if (providerId === "pp_razorpay_razorpay") {
      return {
        ...
        icon: <span className="text-xs font-bold px-1">₹</span>,
      }
    }
    return { title: providerId, icon: null }
  }
