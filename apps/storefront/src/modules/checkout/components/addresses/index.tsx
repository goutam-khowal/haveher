...
const Addresses = ({
  ...
  return (
    // 👑 FIXED GLOBAL CONTAINER: White box layout consistency with matching colors and uniform spacing
    <div className="bg-white px-4 py-6 md:p-8 rounded-2xl border border-pink-50/50 shadow-sm font-sans text-[#3A1A2A]">
      <div className="flex flex-row items-center justify-between mb-6">
        <Heading
          level="h2"
          className="flex flex-row text-xl font-serif italic font-medium tracking-wide gap-x-2 items-baseline text-[#3A1A2A]"
        >
          Shipping Address
          {!isOpen && <CheckCircleSolid className="text-[#D45C88] w-4 h-4" />}
        </Heading>
        {!isOpen && cart?.shipping_address && (
          <Text>
            <button
          ...
          <div className="pb-4">
            ...
            {!sameAsBilling && (
              <div>
                <Heading
                  level="h2"
                  className="text-xl font-serif italic font-medium tracking-wide gap-x-4 pb-6 pt-8 text-[#3A1A2A]"
                >
                  Billing address
                </Heading>
            ...
            <SubmitButton
              className="mt-6 bg-[#3A1A2A] hover:bg-[#D45C88] text-white rounded-full transition-colors h-12 px-8 uppercase tracking-widest text-xs font-bold cursor-pointer shadow-md"
              data-testid="submit-address-button"
            >
              Continue to delivery
            </SubmitButton>
            ...
            {cart && cart.shipping_address ? (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full text-left">
                ...
                >
                  ...
                  <Text className="txt-medium text-gray-600 mt-0.5 leading-relaxed">
                    {cart.shipping_address.address_1}{" "}
                    {cart.shipping_address.address_2}
                  </Text>
                  <Text className="txt-medium text-gray-500">
                  ...
                  <Text className="txt-medium text-gray-400 text-xs uppercase mt-0.5 font-bold tracking-wider">
                    {cart.shipping_address.country_code?.toUpperCase()}
                  </Text>
                </div>
