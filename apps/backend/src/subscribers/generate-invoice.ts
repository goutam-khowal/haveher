...
//   // 👑 CHANGED: Fallback points directly to order.id (Database Order ID) instead of sliced strings
//   const displayId = order.metadata?.custom_invoice_id || order.id;
//   const invoiceFilename = `invoice-${order.metadata?.custom_invoice_id || order.id.slice(-8).toUpperCase()}.pdf`;
...
//     .font("Helvetica")
//     .fontSize(9)
//     .text(`Order ID: ${displayId}`, 300, 75, { align: "right", width: 250 }) // 👑 CHANGED: Title updated to Order ID
//     .text(`Date: ${new Date(order.created_at).toLocaleDateString()}`, 400, 90, {
//       align: "right",
//     });
...
//     // 👑 REFIXED: Dynamic text matching templates fallback structure to format safely
//     const finalFormattedTotal = `INR ${rawTotal.toFixed(2)}`;
...
//       <body>
//         <p>Hi ${customerName}, your order validation details have arrived.</p>
//         <p>Order Reference ID: ${displayId}</p>
//         <p>Total Paid: ${finalFormattedTotal}</p>
//       </body>
//       </html>
//     `;
...
//       from: "HaveHer <onboarding@resend.dev>",
//       to: [finalDestinationEmail],
//       subject: `Your HaveHer Order Confirmation - ${displayId.slice(-8).toUpperCase()}`,
//       html: emailHtmlPayload,
//       attachments: [
//         {
//           filename: invoiceFilename,
...
export default async function orderPlacedInvoiceHandler({
  ...
  // 👑 FIXED: Explicit string conversion checks applied to guarantee string method access
  const rawIdString = String(
    order.metadata?.custom_invoice_id || order.id || "",
  );
  const displayId = rawIdString || "N/A";
  const shortId =
    rawIdString.length > 8
      ? rawIdString.slice(-8).toUpperCase()
      : rawIdString.toUpperCase();
  ...
  doc
    ...
    .font("Helvetica")
    .fontSize(9)
    .text(`Order ID: ${displayId}`, 300, 75, { align: "right", width: 250 })
    .text(`Date: ${new Date(order.created_at).toLocaleDateString()}`, 400, 90, {
      align: "right",
    });
  ...
  // Await file stream write closure successfully
  await new Promise<void>((resolve, reject) => {
    stream.on("finish", () => resolve());
    stream.on("error", reject);
  });
  ...
  try {
    ...
    // 👑 FIXED: Declared and read the dynamic PDF buffer exactly after file writing streams close control
    const pdfBuffer = fs.readFileSync(outputPath);
    ...
    await resend.emails.send({
      from: "HaveHer <onboarding@resend.dev>",
      to: [finalDestinationEmail],
      subject: `Your HaveHer Order Confirmation - ${shortId}`,
      html: emailHtmlPayload,
      attachments: [
        {
          filename: invoiceFilename,
